...
def stamp_envelope(envelope_hash: str) -> Optional[bytes]:
    """
    OTS/Bitcoin anchoring has been permanently disabled (decision: 2026-07-04).
    SafeAgent no longer submits claim envelopes to OpenTimestamps calendar
    servers at claim time.

    This function is kept as a no-op (always returns None) so callers -
    attach_governance_async() below - don't need to change their call
    signature or handling. chain_invariant and admission_invariant (envelope
    hash + BIP-340 signature) are unaffected; only anchoring_invariant is now
    always absent, the same "no anchor yet" state callers already handled
    for the case where OTS submission failed.
    """
    log.debug(
        "OTS anchoring disabled - skipping calendar submission for %s",
        envelope_hash[:16] if envelope_hash else "",
    )
    return None
...
def verify_claim_receipt(receipt: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if receipt.get("ots_proof_hex"):
        ...
        errors.append("anchoring: OTS anchoring is disabled (not configured) - expected, non-fatal")
