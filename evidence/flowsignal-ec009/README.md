...
**Do not run this path until Graham has reviewed and cleared the exact candidate. No Stripe request has been made for this candidate.** The operator supplies `STRIPE_SECRET_KEY` to the process environment, never to the agent or CLI arguments. The runner refuses anything except `sk_test_` and requires `--review-cleared`. That flag is the operator's assertion of clearance, not a technical proof that Graham authorized a run.
...
Before clearance, run `python SafeAgent/evidence/flowsignal-ec009/test_stripe_test_full_path.py --flowsignal-root FlowSignal` for the complete runner and verifier through a fake Stripe module with sockets blocked. It covers the authenticated-account mismatch, one create, retrieval of the durable ID, replay denial, uncertain outcomes and journal preservation, semantic verification, and coordinated provider-record mutation. The fake can produce a locally consistent `stripe-test` bundle: this is **not** evidence of actual Stripe execution. `test_stripe_test_path.py` additionally checks the adapter alone. `test_claim_integrity.py <simulated-bundle> --flowsignal-root FlowSignal` repeats Graham's earlier hostile mutation.
