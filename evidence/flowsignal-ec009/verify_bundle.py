...
def main():
    ...
    try:
        ...
        check(manifest['schema'] == 'safeagent.flowsignal-ec009-offline.v2', 'wrong schema')
        check(manifest['mode'] == 'simulated' and manifest['flowsignal_commit'] == FLOW_COMMIT, 'wrong mode/revision')
        ...
        check(read('provider_account') == {'account_id': ACCOUNT, 'source': 'SIMULATED'},
              'simulated provider account mismatch')
        ...
        check(first['result'] == json.loads(boundary['result_json']) == provider,
    ...
    print(json.dumps({'mode': 'simulated', 'passed': not problems, 'problems': problems}, indent=2))
