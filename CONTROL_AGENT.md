...
1. Inspect current main, open Control PRs, the latest sanitized Control source/status, and last agent report or draft PR. Pick the highest-impact **unresolved** Control issue. Skip any issue already under active review unless adding a new failing test or fixing a demonstrated defect on that same branch.
...
4. If source, evidence, or permissions are missing, or no meaningful defect remains, report **NO JUSTIFIED CHANGE** with the reason and needed evidence. Do not open a filler PR or churn tests and copy.
